...
export const clearProfileFromCookie = () => {
...
export const handleSelectLatestRefreshToken = (request: NextRequest) => {
  const refreshToken = request.cookies.get("refresh-token")?.value
  let newRefreshToken: string | undefined = refreshToken

  const retrieveTokenFromHeader = () => {
    const setCookieHeader = request.headers.get("set-cookie")
    const refreshTokenMatch = setCookieHeader?.match(/refresh-token=([^;]*)/)
    return refreshTokenMatch ? refreshTokenMatch[1] : undefined
  }

  const getRefreshFromSetHeader = retrieveTokenFromHeader()

  if (refreshToken) {
    if (getRefreshFromSetHeader) {
      newRefreshToken = retrieveTokenFromHeader()
    }
  } else {
    newRefreshToken = retrieveTokenFromHeader()
  }
